...
- 选择视觉转录、多模态推理、embedding 模型或向量数据库的具体产品。
- 定义 Android 页面视觉设计。
...
`SourceContext` 是一次用户提交的证据包，包括截图、视觉转录结果、补充文字、来源元数据和分析过程中的说话人/消息结构。本 RFC 中的视觉转录是逻辑能力，不要求使用独立 OCR；MVP 由 ADR-0001 决定仅使用多模态模型实现。

- 原始截图只用于当前处理，不进入长期记忆，也不用于训练。
- 云端处理完成后删除原始截图；长期对象只保存必要的来源摘录、位置引用和不可逆内容指纹。
- 视觉转录文本不是天然事实，必须保留识别置信度和与截图区域的对应关系。
- 用户补充文字与视觉转录内容分别标记来源，不能混为同一种证据。
...
删除提交时，删除其临时原图、视觉转录、结构化提取和以该提交为唯一来源的记忆；包含其他有效来源的记忆移除该来源并重新评估是否仍可保留。相关 InsightHistory 同步删除或去标识化。
...
- 日志不得包含原始截图、完整视觉转录、联系人明文或 memory 正文。
...
以下事项应在实现前以独立 ADR 或 RFC 固定。其中客户端、服务端、AI 编排、认证、主存储与 MVP 检索方案已由 [ADR-0001](../adrs/0001-technology-stack.md) 固定；未决的实现细节仍需后续决策。

- Calendar/Contacts Provider adapter 的具体接口与设备兼容策略。
- Supabase RLS 的连接池上下文、数据库角色和策略测试方案。
- 可替换 MemoryProvider 的具体接口及未来语义检索准入指标。
- 多模态模型评测集、生产模型选择和切换规则。
