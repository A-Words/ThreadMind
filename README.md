...
- `server/`：Fastify API、独立 Worker、LangGraph 提取图、可替换的 Responses 多模态 adapter、带类型/大小/指纹校验的截图提交、私有 Supabase Storage、PostgreSQL-backed 分析任务、可检索 Memory、持久 Insight History、账户导出/删除、Supabase JWT 验证、Kysely repositories、强制 RLS migration，以及 API/Worker 两个 Docker targets。
- `android/`：Compose 客户端、Supabase 邮箱密码与六位 OTP 登录、App 内密码找回/设置、带 Bearer Token 的 Retrofit 客户端、可持久核对的转录与置信度、可搜索/修订/删除/清空的 Memory Center、Insight History、系统分享入口、数据导出与删除控制、完整卡片字段审核、设备目标账户选择，以及带会议冲突、重复联系人和更新字段差异预检的 Calendar/Contacts Provider executor。
...
运行方式、已实现范围及尚未接入的生产能力见 [实现说明](docs/IMPLEMENTATION.md)，逐项验收证据见 [PRD 覆盖矩阵](docs/PRD-COVERAGE.md)。PRD、RFC 和 ADR 仍是后续实现的约束来源。
